        ...
        if emiten_jk != ".JK":
            ...
            if symbol.get('longName') is not None:
                ...
                if profil == "Long Term":
                    ...
                    pengurang = len(for_index)
                    selisih_hari = len(for_index)
                    histor = datumz[['Close']]
                    histor = histor.iloc[-selisih_hari:]
                    ...
                    benerin = datumz["Close"].iloc[-selisih_hari:]
                    benerin.index = for_index["Index"]
                    after_forecastz = pd.concat([benerin, forecastz])
        ...
        if sector != "Sectors 👇🏼":
            if profil == "Short Term":
                ...
                with st.spinner('Wait for it... (it might take a while)'):
                    ...
                    for k in kode:
                        try:
                            datumz = yf.download(k, datetime(2022, 1, 1))
                            datumz = datumz.to_period('D')
